...
>
> Status: `Accepted`; Milestone 2 is `Verified` at `734d151`; deterministic M3B.1-M3B.4 behavior is `Verified` at `7d6c349` and `6ba208a`; M3B.5 is `Verified` at `39885e4`; a follow-up latency optimization is `Implemented` but uncommitted, and the final live release gate remains open. This document does not authorize staging or commits by itself.
...
> Status: the deterministic workflows, live factory, OpenRouter transport, pressure evaluator, debugger-controlled runtime selector, expanded model profiles, semantic oracle v2, denominator-separated latency report, and workflow-neutral workload manifest are `Verified` through `39885e4`. The follow-up latency optimization is `Implemented` but uncommitted. The final live release gate remains open pending its own commit and clean rerun.
...
**Current gate evidence:** The commit-bound suite at `39885e4` includes localhost Playwright/server checks, seller-visible convergence to the exact switched profile name/version, and a delayed seller-session regression proving old-show controls are inert until the new snapshot arrives. It covers Luna standard `none`, Luna standard `low`, Luna priority `none`, Gemini 3.7 Flash `low`, and Gemini 3.5 Flash-Lite `minimal`; transport tests distinguish direct-OpenAI `service_tier` from OpenRouter reasoning. The v2 workload retains explicit semantic contracts and separate all-event, answerable-parent, model-backed, R2, and R3 latency denominators. Follow-up latency and DBG-023 corrections are `Implemented` in the uncommitted tree: closed deterministic scope gates admit exactly the 72 answerable parents to `one_call_template`, the model projection omits non-semantic authority/provenance metadata, typed Python resolution maps natural-language sizes to one trusted bound-listing variant for both workflows, exact availability projects one record, general availability projects one aggregate record rather than the full variant array, and scheduler limits are five/show and fifteen/global. The dirty-tree deterministic suite passes with `351 passed, 5 deselected`; it is not commit-bound `Verified` evidence. The earlier exploratory artifact `runs/exploratory/openrouter_gemini_3_5_flash_lite_minimal_evidence_variant_c5_one_call_v2.json` passed with 72/72 semantic correctness, 14/14 exact variants, all hard invariants zero, zero SLO misses/timeouts, 976.73 ms queue p95, and 1,848.92 ms answerable-parent p95. Because the artifact predates the completed DBG-023 code and identifies `39885e4`, it remains diagnostic only.
...
> Status: M3B.5 is `Verified` at `39885e4` with `300 passed, 5 deselected in 52.29s`. A follow-up latency optimization is `Implemented` but uncommitted. Its dirty-tree Gemini Flash-Lite diagnostic passes the live semantic, safety, and answerable-parent p95 gates, but final evidence remains blocked until that behavior has its own builder-approved commit and the clean committed tree is rerun.
