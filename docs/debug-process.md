...
- **Log, trace, screenshot, or artifact:** The RED pytest outputs were captured in the implementation task. The dirty-tree design and task record is `docs/plans/2026-08-18-deterministic-variant-resolution.md`. No provider credential, model transcript, or full inventory array is retained as evidence.
...
- **Log, trace, screenshot, or artifact:** Accepted design and executable task record: `docs/plans/2026-08-18-demo-reset-design.md` and `docs/plans/2026-08-18-demo-reset-implementation.md`. Browser and API evidence is retained in the focused pytest cases; no customer or external marketplace data is involved.
...
- **Files and functions changed:** `src/sidestage/marketplace/demo_reset.py`; reset/mutation endpoints and snapshot projection in `src/sidestage/app.py`; `RuntimeSelector.reset`; `PreparedChatSource.reset`; `copilot_projection` and `_chat_timeline`; seller `index.html`, `app.js`, and `styles.css`; focused integration and Playwright tests; PRD/TDD/milestone/design documentation.
...
- **Date and commit:** 2026-08-18; reproduced from clean worktree `/tmp/sidestage-m3b6-b5823cc` at `b5823cc`; fixed in the current uncommitted implementation sub-milestone.
- **Status:** Fixed, not commit-bound `Verified`.
...
- **Remaining risk:** The fresh suite emits a Starlette warning that its `httpx`-based `TestClient` path is deprecated in favor of `httpx2`; it does not fail today but should be tracked before the next major test-client upgrade. The fix and clean results are still dirty-tree evidence until they receive a separately reviewed commit and are rerun from that commit.
...
- **Date and commit:** 2026-08-18; discovered while verifying DBG-025 under the clean CPython 3.13.11 environment; fixed in the current uncommitted implementation sub-milestone.
- **Status:** Fixed, not commit-bound `Verified`.
...
- **Remaining risk:** The bounded cooperative probe is synthetic scheduler evidence, not a latency measurement and not evidence that a provider sustains 15 concurrent requests. Only the final live pressure run may establish the release p95 and provider behavior. M3B.6 remains paused until this fix is committed and rerun from a clean committed tree.
...
- **Incident state:** Fixed in the dirty worktree on 2026-08-18.
- **Evidence maturity:** `Implemented`, not `Verified` or `Measured`. No commit contains the combined fix yet.
...
- **Remaining risks:** This is synthetic trusted-catalog behavior and assumes the bound listing's variant/inventory set is complete. The five-second window is an accepted prototype threshold, not marketplace-derived production evidence. No live provider pressure rerun has measured the latency effect, and none of these results become commit-bound `Verified` evidence until the builder approves a commit message, the exact diff is committed, and the clean committed tree is rerun.

### DBG-028 — Concurrent SSE listeners could violate Python 3.9 condition-lock ownership

- **Incident state:** Fixed in the dirty worktree on 2026-08-18.
- **Evidence maturity:** `Implemented`, not `Verified`. The passing commands are not commit-bound.
...
- The M3B eight-stage debugger reads persisted runtime observations. M3B.1-M3B.4 are `Verified` at `7d6c349` and `6ba208a`; M3B.5 is `Verified` at `39885e4`. The follow-up latency optimization and its passing Gemini Flash-Lite artifact remain dirty-tree `Implemented` diagnostics rather than final `Measured` release evidence.
