...
Current state: both closed workflows defined by the M3.1-M3.4 compatibility map are committed in `7d6c349`, the original replay/evaluation support is committed in `6ba208a`, and the M3B.5 runtime selector is committed in `39885e4`. The exact commit-bound command `uv run pytest -q` passed at `39885e4` with `300 passed, 5 deselected in 52.29s`, so its deterministic/browser behavior is `Verified`. Follow-up latency and DBG-023 corrections are `Implemented` but uncommitted: deterministic closed-scope gates reduce the fixed workload from 135 to 72 model calls, the one-call projection excludes non-semantic metadata, exact natural-language size questions resolve to one bound-listing variant in Python, general availability uses one aggregate record instead of all inventory rows, and the measured candidate scheduler is five workers per show and fifteen globally. The dirty-tree deterministic suite passes with `351 passed, 5 deselected`; this is not commit-bound `Verified` evidence. One earlier dirty-tree exploratory Gemini 3.5 Flash-Lite `minimal` cell passed with 72/72 semantic correctness, all hard invariants zero, no SLO misses or hard timeouts, and 1,848.92 ms answerable-parent p95. That artifact predates the final DBG-023 code and is not final `Measured` evidence. M3B.6 remains pending an approved optimization commit and a clean committed-tree rerun.
