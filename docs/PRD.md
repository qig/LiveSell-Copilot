...
> Status: `Accepted` — builder-approved v1 design; Milestone 2 and deterministic M3B.1-M3B.4 behavior are `Verified`; the M3B.5 Optimization and Debug Session extension is `Implemented` in the current uncommitted tree but not yet commit-bound `Verified`, and the live release gate is open
>
...
| --- | --- | --- |
| Grounded-suggestion latency | Answerable-parent end-to-end p95 under 2 seconds | Gate remains open. Legacy live artifacts report 3,414.37 ms one-call and 4,530.28 ms two-call all-event workload p95. The current evaluator now reports answerable-parent, model-backed, R2, R3, and all-event denominators separately and uses answerable-parent p95 for this gate; a new live rerun is pending. |
...
| Runtime selection isolation | Every accepted question uses exactly one pinned workflow, model profile, and configuration version from acceptance through publication | `Implemented`; pre-commit deterministic coverage passes, commit-bound `Verified` evidence pending |
...
| Answerable semantic correctness | At least 95% produce a broker-accepted suggestion with the expected category, evidence fact type, exact variant when applicable, and—on one-call—the expected approved template | Current scripted evaluator: 72/72 for both workflows. Legacy live results of 66/72 one-call and 54/72 two-call measured broker acceptance only and cannot be retroactively promoted to semantic correctness; new live reruns are pending. |
...
Current state: both closed workflows defined by the M3.1-M3.4 compatibility map are committed in `7d6c349`, and the original replay/evaluation support is committed in `6ba208a`. M3B.5 is implemented in the current uncommitted tree: startup builds a sanitized closed model/workflow catalog; debugger switches are versioned and pinned at chat acceptance; traces, cards, R2/R3 receipts, and the read-only marketplace badge retain that identity. Model profiles now distinguish reasoning effort from direct-OpenAI service tier and include Luna `none`, Luna `low`, Luna priority `none`, Gemini 3.7 Flash `low`, and Gemini 3.5 Flash-Lite `minimal` candidates. The workload manifest is workflow-neutral, all 72 answerable parents have semantic oracle contracts, scripted runs score 72/72 semantic correctness, and latency reports distinct all-event, answerable-parent, model-backed, R2, and R3 denominators. Legacy live artifacts still favor one-call Luna on broker acceptance and all-event p95, but they predate those evaluator corrections and are not semantic or final SLO evidence. M3B.6 remains pending a passing live cell, an approved commit, and clean final reruns.
