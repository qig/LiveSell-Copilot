...
6. The concrete workflow/model selection pinned when the question is accepted runs to completion. Before inventory retrieval, Python parses trusted variant labels and buyer wording into size system, audience, and decimal size. One matching candidate resolves to its trusted variant ID; multiple compatible candidates are `ambiguous`; incomplete or implausible wording is `missing_evidence`. When system and audience are explicit or can be inferred because all compatible trusted candidates agree, a plausible exact size with no catalog match becomes one trusted zero-availability fact rather than a guess. `one_call_template` constructs one immutable task from the question and bounded candidate evidence: at most one exact or negative variant record, one aggregate availability summary for general questions, or no inventory record. Its registered `EvidenceTemplateAgent` makes one call and selects evidence IDs plus exactly one approved template, `needs_seller`, or `no_response`. `two_call_draft` runs its registered `EvidencePlannerAgent`, whose strict contract has no variant-label output, deterministic targeted retrieval, and registered `ReplyDrafterAgent`. A later debugger switch affects only newly accepted questions and cannot mix configurations inside an in-flight trace. Application code validates every terminal and renders template text from the selected trusted evidence. Neither workflow exposes database, direct-send, or marketplace-mutation tools.
...
- **R2 — Seller-approved send:** Internally names the seller-approved path. The seller UI calls this **Manual review**.
- **R3 — Bounded auto-reply:** Internally names the versioned automatic-send capability. The seller UI calls this **Auto-message**, enables it by default for new/reset demo shows, and offers an immediate switch to Manual review.
- **R4 — Open-ended auto-reply:** Out of scope.

The seller workspace never displays the internal R2/R3 labels. Switching to Manual review prevents new automatic sends; replies already sent remain in the audit history. Auto-message never grants authority for marketplace actions. Operational actions use the authority matrix above rather than an AI automation ladder.

Auto-message may send any current supported answer category only after the deterministic broker has reduced the reply to one trusted factual basis and validated scope, freshness, evidence support, tone, and canonical-question uniqueness. Supported facts include:

- Current displayed price; exact variant availability or deterministic aggregate availability.
- Shipping, payment, and return policies.
- Listing identity, release date, MSRP, materials, sizing guidance, authenticity, and condition.

Auto-message never sends unrestricted model prose. Application code renders the automatic reply from the one verified typed fact and seller-approved tone behavior. On `one_call_template`, application code also renders the review suggestion from a versioned approved template. Seller edits remain exact seller-authored text. Free-form model prose exists only in the retained `two_call_draft` benchmark baseline and may appear only in Manual review.

Offers, negotiation, discounts, markdowns, customer-specific order issues, ambiguous questions, uncertain active-SKU matches, missing/conflicting evidence, and stale results remain in Manual review or **Needs seller**. They are never guessed or auto-sent.

Immediately before an Auto-message send, SideStage rechecks the capability version plus the bound listing epoch, active SKU, and exact evidence version. If a normal in-flight candidate's listing is no longer active, SideStage suppresses it and moves the question to **Needs seller** with the previous SKU. A question already known to reference a previous listing instead receives a deterministic application-owned current-stage notice with zero model calls; Auto-message sends the notice, while Manual review holds it as a suggestion. Other version mismatches prevent automatic send and move the question to **Awaiting review** with refreshed card facts.
...
All raw messages remain observable. Reactions, high-certainty obvious noise, closed v1 ambiguity or unsupported patterns, and explicit authority/prompt-injection attempts exit through deterministic typed routes before provider work. Event-ID replay remains idempotent indefinitely; normalization-equivalent text shares a canonical question only inside a five-second seller/show/listing-epoch window, so the same question asked ten seconds later is independently answerable. Uncertain messages may still reach the bounded analysis call. No event is silently discarded.
...
- Per-show **Auto-message** / **Manual review** toggle. Auto-message is the default for new and reset demo shows, with a persistent enabled-state warning and an immediate switch to Manual review.
...
A duplicate question inside the five-second grouping window displays **Grouped** and links to its canonical question. A question whose bound listing is already known to be inactive displays a compact **Previous listing** tag plus the previous SKU and receives an application-owned notice naming the item now on stage; it makes no model call. Auto-message sends the notice, while Manual review displays it as a suggestion. An in-flight generated answer invalidated by a listing change still enters **Needs seller**. **Unanswered** becomes terminal only when the seller dismisses the question or the show ends. Each card otherwise shows only SKU, current price, referenced variant plus stock, and an applicable policy line when relevant.
...
Current state: both closed workflows defined by the M3.1-M3.4 compatibility map are committed in `7d6c349`, the original replay/evaluation support is committed in `6ba208a`, and the M3B.5 runtime selector is committed in `39885e4`. The exact commit-bound command `uv run pytest -q` passed at `39885e4` with `300 passed, 5 deselected in 52.29s`, so its deterministic/browser behavior is `Verified`. Latency and DBG-023 corrections are committed in `12f3bab`: deterministic closed-scope gates reduce the fixed workload from 135 to 72 model calls, the one-call projection excludes non-semantic metadata, exact natural-language size questions resolve to one bound-listing variant in Python, general availability uses one aggregate record instead of all inventory rows, and the measured candidate scheduler is five workers per show and fifteen globally. Their retained live artifact predates that final commit and they still await M3B.6 clean-tree verification. Commit `b5823cc` contains the seller-workspace reset/operability extension: the synthetic UI enforces an active epoch, resets safely, partitions open questions, scrolls independently, and renders quoted durable replies. The later lifecycle, pressure, Auto-message/routing, and SSE fan-out corrections are `Implemented` but uncommitted. The current combined dirty-tree full suite passes with `368 passed, 5 deselected in 80.61s` on the project Python 3.9 environment and `368 passed, 5 deselected, 1 warning in 44.16s` on Python 3.13/FastAPI 0.141.1; this is not commit-bound `Verified` evidence. A live diagnostic exercised `9 for man` through the one-call Luna path and all eight stages, switched the same show to `two_call_draft`, then produced one manual-review and one automatic reply with quoted timeline entries; this is compatibility evidence, not a latency measurement. One earlier dirty-tree exploratory Gemini 3.5 Flash-Lite `minimal` cell passed with 72/72 semantic correctness, all hard invariants zero, no SLO misses or hard timeouts, and 1,848.92 ms answerable-parent p95. That artifact identifies `39885e4`, predates the final DBG-023 code, and is not final `Measured` evidence. M3B.6 remains pending a builder-approved commit and a clean committed-tree rerun.
